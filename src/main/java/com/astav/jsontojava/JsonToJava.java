...
/**
 * User: Astav Date: 10/18/12
 * <p/>
 * A very crude JsonToJava source file generator that tries to discover the schema and generate the necessary java classes.
 * <p/>
 * Tips: Specify all values for json fields with non-null values. By doing so, the generator will try to re-use classes it already generated in other structures
 * as long as the class can be parsed back from json into the generated class.
 * <p/>
 * Limitations: - Can't discover and create abstract types - Can't collate unspecified fields across different structures into the same class when missing
 * information - Doesn't support byte, short and char types yet
 */
public class JsonToJava {

	public static void mainOld(String[] args) throws IOException, ClassNotFoundException {
		ObjectMapper mapper = new ObjectMapper();
		System.out.println("JsonToJava v0.1");
		System.out.println();
		if (args.length < 6) {
			System.out
					.println("com.astav.jsontojava.JsonToJava <json-file> <output-directory> <package-name> <main-class> <regex-file> <import-base-dir> <import-package-names>");
			...
			System.out
					.println("  <prompt-for-complex-value-types> (required - 'true' or 'false'): when encountering a map of maps, instead of generating a new class for every entry, prompt instead");
		...
		Map<String, Object> map = mapper.readValue(FileUtils.readFileToString(new File(args[0])), Map.class);
	...
	private static void convertJsonToJava() throws IOException, ClassNotFoundException {
		ObjectMapper mapper = new ObjectMapper();
		...
		String packageName = "com.bd.rs.domain";
		...
		@SuppressWarnings("unchecked")
		Map<String, Object> map = mapper.readValue(FileUtils.readFileToString(new File(jsonFile)), Map.class);
	...
	private static void convertJavaToJson() throws IOException, ClassNotFoundException {
		...
		ObjectMapper mapper = new ObjectMapper();
		mapper.writeValue(System.out, userProfileEntry);
